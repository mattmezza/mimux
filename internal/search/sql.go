...
func BuildLocalSQL(q *SearchQuery, scope Scope, account string, folderID int64) (where string, args []any) {
	...
	for _, t := range q.Terms {
		...
		case "body": // ponytail: body: is server-only; locally we match the snippet as best effort.
			if t.Neg {
		...
		case "has":
			...
			case "link": // ponytail: link detection is a snippet substring scan, good enough for the list preview.
