...
function hdrScroll(pane, scroller, y, reclaim) {
  ...
  // ponytail: 240ms = the 200ms CSS transition + a frame of slack, tune if
...
window.previewAttachment = function (btn, url, kind) {
  ...
  // ponytail: an iframe fires load even for an error page, so a failed fetch
...
(function () {
  ...
  document.addEventListener("touchstart", (e) => {
    ...
    // ponytail: "the list was still moving" == a scroll event fired in the last
...
document.addEventListener("keydown", (e) => {
  ...
  if (t instanceof HTMLElement && (t.isContentEditable || ["INPUT", "TEXTAREA", "SELECT"].includes(t.tagName))) {
    ...
    // ponytail: an input inside an open "⋯"-style popover (the label add-form
