...
(function () {
  ...
  //
  // NOTE: a REFRESH, not a sync. The POST /refresh on visibility that used
...
// Delegated from document, NOT bound to the element: htmx replaces the whole
// body on history restore (Back out of a search or an open message — the usual
// way to navigate on mobile), which drops any listener bound to the old pill
// node and leaves the pill dead for the rest of the page's life.
...
// mobile. Handle touchend too; preventDefault there suppresses the synthesized
// click that would otherwise double-fire cycleScope().
document.addEventListener("click", (e) => {
  if (e.target.closest?.("#scope-pill")) cycleScope();
});
document.addEventListener("touchend", (e) => {
  if (!e.target.closest?.("#scope-pill")) return;
  e.preventDefault();
  cycleScope();
});
...
// Strip the quote formats generated by compose before looking for attachment
// hints. An attribution is a quote only when it introduces >-prefixed text, so
// a sender's own "On Monday, I wrote:" is preserved.
function stripQuotedText(text) {
  const htmlQuote = /<blockquote\b/i.exec(text);
  if (htmlQuote) text = text.slice(0, htmlQuote.index);

  const forwardQuote = /^[ \t]*---------- Forwarded message ----------[ \t]*(?:\r?\n|$)/im.exec(text);
  if (forwardQuote) text = text.slice(0, forwardQuote.index);

  const replyQuote = /^[ \t]*On .+ wrote:[ \t]*\r?\n(?:[ \t]*\r?\n)*[ \t]*>/im.exec(text);
  if (replyQuote) text = text.slice(0, replyQuote.index);
...
// NOTE: keys typed while a body *iframe* has focus never reach us — click
